...
public class KitchenPanel extends JPanel {
    ...
    public KitchenPanel() {
        ...
        orderTable.setDefaultRenderer(Object.class, new StatusCellRenderer());
                ...
                orderTable.setBackground(new Color(255, 250, 240)); // light cream for rows
        orderTable.setForeground(Color.BLACK);

        orderTable.getTableHeader().setBackground(new Color(210, 180, 140)); // tan header
    ...
    private void fetchOrders() {
        ...
        try {
            ...
            if (orders.length() > 0) {
                int newestOrderId = orders.getJSONObject(0).getInt("order_id");

                if (lastSeenOrderId == -1) {
                    // First load: just set it without playing sound
                    lastSeenOrderId = newestOrderId;
                } else if (newestOrderId != lastSeenOrderId) {
                    // Play sound ONLY if a new order comes in
                    playAlertSound();
                    lastSeenOrderId = newestOrderId;
                }
            }
    ...
    public static void playAlertSound() {
        try {
            File soundFile = new File("src/DAD/Alert.wav");
